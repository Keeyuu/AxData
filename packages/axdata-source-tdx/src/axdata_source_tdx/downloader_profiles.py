...
def tdx_downloader_profiles(
    ...
    def index_kline_daily_profile() -> Any:
        return downloader_profile_cls(
            ...
            default_connection_mode="long_connection",
            concurrency=tdx_single_kline_concurrency,
            primary_key=("instrument_id", "trade_time"),
            resource_group="tdx.quote",
            adapter_factory="tdx",
            runtime_source_server_max_factory="tdx",
            default_output_path_parts=_core_table_output_path_parts("index_daily"),
            snapshot_date_meta_keys=["trade_date", "data_date", "snapshot_date", "date"],
            file_stem_template="{interface_name}_{snapshot_date}",
            required_columns=["instrument_id", "trade_time", "period"],
            expected_columns=[
            ...
            numeric_positive_columns=["open", "high", "low", "close", "volume", "amount"],
            params=[
                ["code", "string/list", "是", "指数代码：000001.SH/399001.SZ/sh000001"],
                ["period", "string", "否", "K线周期：day 至 year 及 1m/5m/15m/30m/60m，默认 day"],
                ["count", "integer", "否", "返回最近 K 线数量；默认 120，最大 65535"],
                ["full_history", "boolean", "否", "翻页拉取指数全部历史，忽略 count 截断"],
            ],
            description="保存显式指数代码的日 K 线全历史，默认 full_history=true 翻页拉全历史。",
        )

    def index_codes_profile() -> Any:
        return downloader_profile_cls(
            interface_name="index_codes_tdx",
            display_name="指数列表",
            downloader_type="full_snapshot",
            default_params={"include_tdx_block_index": True},
            default_fields=[
                "instrument_id",
                "symbol",
                "tdx_code",
            ...
            adapter_factory="tdx",
            runtime_source_server_max_factory="tdx",
            default_output_path_parts=_tdx_output_path_parts("index_codes_tdx"),
            required_columns=["instrument_id"],
            expected_columns=[
                "instrument_id",
                "symbol",
            ...
            numeric_positive_columns=["previous_close"],
            params=[
                ["name", "string/list", "否", "指数简称过滤，例如 上证指数、沪深300"],
                ["code", "string/list", "否", "指数代码过滤：支持 000001.SH、399001.SZ、sh000001"],
                ["exchange", "string/list", "否", "交易所过滤：all、SSE、SZSE、BSE；默认 all"],
                ["include_tdx_block_index", "boolean", "否", "是否包含通达信板块/行业/题材指数"],
            ],
            description="一次性拉取通达信指数列表（默认含板块/行业/题材指数）并保存为本地快照。",
        )
