...
from tests.tdx_plugin_helpers import ensure_local_tdx_plugin_paths
...
class FakeSession:
    ...
    def post(self, url, json, headers=None, timeout=None):
        ...
        if table == "stock_basic_exchange":
            ...
            records = [{"instrument_id": "000001.SZ", "trade_time": "20240102", "close": 10.2}]
        return FakeResponse(
            {
                "success": True,
...
def test_sdk_daily_posts_v1_query_and_returns_dataframe():
    ...
    assert session.calls[0]["json"]["fields"] == ["instrument_id", "trade_time", "close"]
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            {"code": "000988.SZ"},
            ["instrument_id", "auction_time", "price"],
        ),
        (
        ...
        (
            ...
            {"code": "000001.SZ"},
            ["instrument_id", "updated_date", "eps"],
        ),
        (
            "stock_finance_profile_tdx",
            {"code": "000001.SZ"},
            ["instrument_id", "updated_date", "ipo_date"],
        ),
...
def test_sdk_local_session_uses_core_source_session_without_http(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        axdata_core, "create_source_session", fake_create_source_session, raising=False
    )

    client = ax.AxDataClient(data_root=tmp_path / "data", session=session)
    with client.session(
        source="tdx", source_server_count=4, connections_per_server=2
    ) as source_session:
    ...
    assert rank.to_dict(orient="records") == [{"interface": "stock_realtime_rank_tdx", "value": 3}]
...
def test_sdk_local_call_uses_core_source_request_gateway_without_http(tmp_path, monkeypatch):
    ...
    def fake_request_interface(
        interface, *, params=None, fields=None, persist=False, options=None, data_root=None
    ):
        calls.append(
            {
                "interface": interface,
...
def test_sdk_api_mode_is_selected_by_api_base():
    ...
    df = client.daily(instrument_id="000001.SZ", fields=["instrument_id", "close"])
